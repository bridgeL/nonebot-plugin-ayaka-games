...
from ayaka import AyakaBox, BaseModel, logger, load_data_from_file, Numbers
...
@box.on_text(states="选择游戏")
async def set_mode(nums: list[float] = Numbers()):
    '''从可选值中选择一项'''
    if not nums or int(nums[0]) not in questions_bin:
        await box.send("输入不合法")
        return

    cache = box.get_data(Cache)
    cache.n = int(nums[0])
    await box.send(f"选择游戏为 {cache.n}点")
    box.state = "计算"
    await select_question()


@box.on_cmd(cmds=["出题", "下一题", "next"], states="计算")
...
@box.on_cmd(cmds=["题目", "查看题目", "查看当前题目", "当前题目", "question"], states="计算")
async def show_question():
    '''展示题目'''
    cache = box.get_data(Cache)
    await box.send(cache.question)
    await box.send(f"TIPS：本题至少有{len(cache.solutions)}种答案（使用不同的运算符）")


@box.on_cmd(cmds=["答案", "answer"], states="计算")
...
@box.on_cmd(cmds="测试24点", always=True)
async def test_exp():
    '''<表达式> 请使用英文括号并闭合，不要有空格，次方使用**，不要使用小数，因为此命令仅为测试中缀表达式计算使用，不做任何纠错'''
    exp = str(box.arg)
    try:
        r = calc(exp)
    except Exception as e:
